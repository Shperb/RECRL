...
from cmdps_via_bvf.models.grid_model import OneHotDQN, OneHotValueNetwork


from cmdps_via_bvf.common.schedules import LinearSchedule
...
class SafeSarsaAgent(object):
    ...
    def __init__(self,
                 args,
                 env,
                 writer = None, cost_lim_curriculum=None):
    ...
    def safe_deterministic_pi(self, state,  current_cost=0.0, greedy_eval=False):
        ...
        with torch.no_grad():
            ...
            if (random.random() > self.eps_decay.value(self.total_steps)) or greedy_eval:
                ...
                constraint_mask = torch.le(cost_q_val + cost_r_val, self.cost_lim + current_cost).float()
